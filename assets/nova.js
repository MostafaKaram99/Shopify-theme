    ...
    var nuranium = {
            ...
            tabslider.each(function () {
                ...
                if (productGrid.not('.slick-initialized')) {
                    productGrid.slick({
                        ...
                        slidesToScroll: (productGrid.data("loop"), productGrid.data("1")),
                        ...
                        responsive: [
                            ...
                            { 
                                ...
                                settings: { 
                                    ...
                                    slidesToScroll: productGrid.data("1") 
                                } 
                            },
                            { 
                                breakpoint: 1200, 
                            ...
                            { 
                                ...
                                settings: { 
                                    ...
                                    slidesToScroll: productGrid.data("1"), arrows: !1 
                                } 
                            },
                        ]
                    });
        ...
        setNovAddedForWishlistIcon: function(ProductHandle) {    
            ...
            if (idxArr >= 0) {
                ...
                wishlistElm.find('.wishlist-text').text('Remove WishList');
            }
            else {
                wishlistElm.removeClass('whislist-added');
                wishlistElm.find('.wishlist-text').text('Add to WishList');
            };
        },
            ...
            jQuery.getJSON(window.router + '/products/'+ProductHandle+'.js', function(product) {
                ...
                if (product.available) {
                    if (product.variants.length == 1) {
                        productHTML += '<button class="hover_x btn btnAddToCart" type="submit" data-form-id="#-'+product.id+'" data-pid="'+product.variants[0].id+'" ><span>'+ theme.strings.addToCart +'</span><i class="zmdi zmdi-long-arrow-right"></i></button><input type="hidden" name="id" value="'+ product.variants[0].id +'" />'; 
                    } 
                    if (product.variants.length > 1){
        ...
        initMiniCart: function() {
          jQuery.getJSON("/cart.js", function (cart) {
            ...
            if (cart.item_count > 0) {
              ...
              html += '<button type="submit" class="hover_x btn cart__checkout" name="checkout"><span>'+ theme.strings.check_out +'</span><i class="zmdi zmdi-long-arrow-right"></i></button>';
              html += '<a href="/cart" class="hover_x btn"><span>'+ theme.strings.view_cart +'</span><i class="zmdi zmdi-long-arrow-right"></i></a>';
              html += '</div>';
              html += '</div>';
              html += '</form>';
              ...
              Currency.convertAll(shopCurrency, $('#currencies span.selected').attr('data-currency'));
            	...
            	html += '<div class="empty_title text-center mt-60">'+ theme.strings.cart_empty +'</div>';
            ...
            $(document).on('click', '.btnChooseVariant', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        $.magnificPopup.open({
                            items: {
                                src: '<div class="nov-with-anim popup-quick-view" id="content_variants">' + data + '</div>',
                                type: 'inline'
                            },
                            removalDelay: 1000,
                            closeMarkup: '<i class="zmdi zmdi-hc-fw zmdi-close"></i>',
